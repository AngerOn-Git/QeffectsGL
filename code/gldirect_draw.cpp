    ...
    _type(type)
{
    ...
    glAssert(gl::qglGenBuffers(1, &_buffer_id));gl::qglGenBuffers
}
    ...
    _type(type)
{
    ...
    glAssert(gl::qglGenBuffers(1, &_buffer_id));
    glAssert(gl::qglBindBuffer(_type, _buffer_id));
    glAssert(glBufferData(_type, nb_elt * _data_ratio, NULL, mode));
    glAssert(gl::qglBindBuffer(_type, 0));
}
...
Shader_prog::~Shader_prog()
{
    ...
    glAssert(gl::qglDeleteProgram(_id));
}
...
public:
    ...
    GlVao() { glAssert(gl::qglGenVertexArrays(1, &_id)); }

    ~GlVao() { gl::qglDeleteVertexArrays(1, &_id); }

    void bind() { glAssert(gl::qglBindVertexArray(_id)); }

    static void unbind() { gl::qglBindVertexArray(0); }
    ...
    void record_attr(GLuint vbo_id, int attr_idx, int nb_components) {
        glAssert(gl::qglBindBuffer(GL_ARRAY_BUFFER, vbo_id));
        glAssert(gl::qglVertexAttribPointer(attr_idx, nb_components, GL_FLOAT, GL_FALSE, 0, 0));
        glAssert(gl::qglEnableVertexAttribArray(attr_idx));
    }

    /// @warning you must bind the vao buffer using this method
    void record_elt(GLuint vbo_elts_id) {
        glAssert(gl::qglBindBuffer(GL_ELEMENT_ARRAY_BUFFER, vbo_elts_id));
    }
...
struct Vec3 {
    ...
    Vec3 operator* (const Vec3& v_) const { return Vec3(x * v_.x, y * v_.y, z * v_.z); }
    bool operator!=(const Vec3& v_) const { return (x != v_.x) | (y != v_.y) | (z != v_.z); }
    ...
    Vec3& operator+=(const Vec3& v_) { x += v_.x; y += v_.y; z += v_.z; return *this; }
    Vec3& operator*=(const Vec3& d_) { x *= d_.x; y *= d_.y; z *= d_.z; return *this; }

    Vec3& operator+=(float f_) { x += f_; y += f_; z += f_; return *this; }
    Vec3& operator/=(float d_) { x /= d_; y /= d_; z /= d_; return *this; }
    Vec3& operator*=(float d_) { x *= d_; y *= d_; z *= d_; return *this; }

    Vec3  cross(const Vec3& v_) const { return Vec3(y * v_.z - z * v_.y, z * v_.x - x * v_.z, x * v_.y - y * v_.x); }
    float dot(const Vec3& v_) const { return x * v_.x + y * v_.y + z * v_.z; }

    float norm_squared() const { return dot(*this); }
    Vec3 normalized() const { return (*this) * (1.f / sqrtf(norm_squared())); }
...
GlDirect_draw::GlDirect_draw(bool use_internal_shader, GLenum buffer_mode) :
...
void GlDirect_draw::init_shader() { Shader_dd::init(); }
void GlDirect_draw::erase_shader() { Shader_dd::clear(); }
...
static
void update_normals_quads(const std::vector<float>& verts,
    std::vector<float>& normals,
    bool is_vert_provok_mode_last)
{
    // look up
    for (unsigned i = 0; i < (verts.size() / 3 / 4); ++i)
    {
        // retrieve face index
...
static
void update_normals_triangle_fan(const std::vector<float>& verts,
    std::vector<float>& normals,
    bool is_vert_provok_mode_last)
{
...
static
void update_normals_quad_strip(const std::vector<float>& verts,
    std::vector<float>& normals,
    bool is_vert_provok_mode_last)
{
...
static
void update_normals_triangles(const std::vector<float>& verts,
    std::vector<float>& normals,
    bool is_vert_provok_mode_last)
{
    for (unsigned i = 0; i < verts.size() / 3 / 3; ++i)
    {
        // retrieve face index
...
static
void update_normals_triangle_strip(const std::vector<float>& verts,
    std::vector<float>& normals,
    bool is_vert_provok_mode_last)
{
    if (verts.size() / 3 >= 3) // at least one triangle
...
void GlDirect_draw::update_normals(Mode_t mode,
    ...
    std::vector<float>& normals)
{
    ...
    switch (mode) {
    case MODE_QUADS:          update_normals_quads(verts, normals, prov_mode); break;
    case MODE_TRIANGLE_FAN:   update_normals_triangle_fan(verts, normals, prov_mode); break;
    case MODE_QUAD_STRIP:     update_normals_quad_strip(verts, normals, prov_mode); break;
    case MODE_TRIANGLES:      update_normals_triangles(verts, normals, prov_mode); break;
    case MODE_TRIANGLE_STRIP: update_normals_triangle_strip(verts, normals, prov_mode); break;
    default: break;
    }
}
...
static
void push(int vert_i, int attr_size,
...
static
void quads_to_tris(int attr_size,
    const std::vector<float>& quad_attr,
    std::vector<float>& tri_attr)
{
    // look up every quads
    tri_attr.reserve(quad_attr.size() * 2);
    for (unsigned i = 0; i < (quad_attr.size() / attr_size / 4); ++i)
    {
        float f[] = { i * 4, i * 4 + 1, i * 4 + 2, i * 4 + 3 };
...
static
void quad_strip_to_tri_strip(int attr_size,
    const std::vector<float>& quad_attr,
    std::vector<float>& tri_attr)
{
...
void GlDirect_draw::convert_prim
(
    int buff_id,
    Mode_t old_mode,
    Mode_t new_mode,
...
void GlDirect_draw::convert_to_triangles(int buff_id)
{
...
void GlDirect_draw::convert_to_triangle_strip(int buff_id)
{
...
void GlDirect_draw::end_update()
{
    ...
    // unbind vbos
    glAssert(gl::qglBindBuffer(GL_ARRAY_BUFFER, 0));
...
void GlDirect_draw::enable_lighting(bool state)
{
...
void GlDirect_draw::draw_buffer(Mode_t mode_t, int i)
{
...
#ifndef USE_GL_LEGACY
    assert_msg(_is_mat_set, "ERROR: you forgot to setup your transformation matrices with set_matrix().");
    ...
    glAssert(gl::qglBindBuffer(GL_ARRAY_BUFFER, 0));
    glAssert(gl::qglBindBuffer(GL_ELEMENT_ARRAY_BUFFER, 0));
